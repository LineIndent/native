...
LIBRARY_ROOT = Path(os.environ.get("components", "."))
UI_DIR = LIBRARY_ROOT / "components" / "ui"
...
def build_registry() -> dict:
    if not UI_DIR.exists():
        raise FileNotFoundError(
            f"No components/ui/ directory found at {UI_DIR.resolve()}.\n"
    ...
    for py_file in sorted(UI_DIR.glob("*.py")):
        ...
        if module_name.startswith("_"):
            continue
        try:
            module = importlib.import_module(f"components.ui.{module_name}")
    ...
    if not registry:
        print(
            f"  [registry] WARNING: found {UI_DIR} but registered zero "
            f"components. Check the per-module skip messages above — your "
            f"real components/ui/*.py files likely failed to import (a "

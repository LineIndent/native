...
//   var MATCH_BASE_VALUE = "__match_base__"; // shared sentinel: both Color and Chart default to tracking Base independently
...
//   // so it decodes with chart defaulting to tracking base. An unrecognized future
...
//     if (version === 1) {
//       // Legacy format — no chart dimension existed yet, defaults to tracking base.
...
//     var baseLabel = baseEntry ? baseEntry.label : "Match base";
//     if (colorSelect) {
...
//     var isDark = document.documentElement.classList.contains("dark");
//     var values;
//     if (state.chart === MATCH_BASE_VALUE) {
//       // Chart's default tracks BASE directly — independent of whatever
//       // Color is currently doing, same as Color's own default independently
//       // tracks Base. Not chained (chart -> color -> base); both are
//       // siblings that separately fall back to base until overridden.
//       var b = findById("base", state.base);
//       values = b ? (isDark ? b.dark : b.light) : {};
//     } else {
...
//   // One dot per select, each keyed to a specific design token: Base shows
//   // its own --accent, Color shows the currently-effective --primary, Chart
//   // shows the currently-effective --chart-1.
//   function syncSwatches() {
//     var isDark = document.documentElement.classList.contains("dark");
//     var baseEntry = findById("base", state.base);
//     var baseValues = baseEntry ? (isDark ? baseEntry.dark : baseEntry.light) : null;

//     var baseSwatch = document.getElementById("base-theme-select-swatch");
//     if (baseSwatch && baseValues && baseValues.accent) {
//       baseSwatch.style.backgroundColor = baseValues.accent;
//     }
...
//         colorValue = baseValues && baseValues.primary;
//       } else {
//         var c = findById("color", state.color);
//         colorValue = c && (isDark ? c.dark.primary : c.light.primary);
//       }
//       if (colorValue) colorSwatch.style.backgroundColor = colorValue;
//     }

//     var chartSwatch = document.getElementById("chart-color-select-swatch");
//     if (chartSwatch) {
//       var chartValue;
//       if (state.chart === MATCH_BASE_VALUE) {
//         chartValue = baseValues && baseValues["chart-1"];
//       } else {
//         var ce = findById("color", state.chart);
//         chartValue = ce && (isDark ? ce.dark["chart-1"] : ce.light["chart-1"]);
//       }
//       if (chartValue) chartSwatch.style.backgroundColor = chartValue;
//     }
//   }
...
//       // Color and Chart are independent siblings that BOTH default to
//       // tracking Base directly (not chained through each other) — so a
//       // new base updates both synthetic options, but only force-selects
...
(function () {
  ...
  // Reads live values back from state + registries so swatches can never
  // drift out of sync with what's really applied. All three swatches
  // deliberately use the SAME token (--chart-1) and always the LIGHT
  // variant — the sidebar chrome they live in is permanently dark-styled
  // regardless of the page's own dark/light toggle, so there's no reason
  // for the swatches themselves to switch with it. chart-1 turned out to
  // be the most visually representative token across every base/color
  // entry (accent/primary skew too pale or too dark depending on the
  // theme), so it's used uniformly rather than a different token per
  // select.
  function syncSwatches() {
    var baseEntry = findById("base", state.base);
    var baseLight = baseEntry ? baseEntry.light : null;

    var baseSwatch = document.getElementById("base-theme-select-swatch");
    if (baseSwatch && baseLight && baseLight["chart-1"]) {
      baseSwatch.style.backgroundColor = baseLight["chart-1"];
    }
    ...
    if (colorSwatch) {
      var colorValue;
      if (state.color === MATCH_BASE_VALUE) {
        colorValue = baseLight && baseLight["chart-1"];
      } else {
        var c = findById("color", state.color);
        colorValue = c && c.light["chart-1"];
      }
      if (colorValue) colorSwatch.style.backgroundColor = colorValue;
    }
    ...
    if (chartSwatch) {
      var chartValue;
      if (state.chart === MATCH_BASE_VALUE) {
        chartValue = baseLight && baseLight["chart-1"];
      } else {
        var ce = findById("color", state.chart);
        chartValue = ce && ce.light["chart-1"];
      }
      if (chartValue) chartSwatch.style.backgroundColor = chartValue;
    }
  }
  ...
  function updateThemeSwatches() {
    ...
    // 1. If we haven't cached the original raw text yet, do it now
    if (!code.dataset.rawText) {
      code.dataset.rawText = code.textContent;
    }

    // 2. Always use the fresh content of the block (or fallback to cache)
    const textContent = code.textContent || code.dataset.rawText;

    // 3. Inject the color circles inline next to the oklch values
    code.innerHTML = textContent.replace(/oklch\([^)]+\)/g, (m) => {
      return `<span style="background-color:${m};width:.85em;height:.85em;display:inline-block;margin-right:.4em;border-radius:3px;border:1px solid rgba(255,255,255,.15);vertical-align:middle;flex-shrink:0;"></span>${m}`;
    });
  }
  ...
  document.addEventListener("click", function (e) {
    ...
    if (e.target.id === "copy-css-preset") {
      ...
      const text = document.getElementById("css-theme-copy-text");
      if (text) {
    ...
    // --- 2. GET CODE / OPEN PANE ACTION ---
    if (e.target.id === "copy-theme-button") {
      var target = document.getElementById("get-css-theme");
      if (target) {
        target.textContent = getThemeCSS();

        // Update the swatches immediately when the text loads!
        updateThemeSwatches();
      }
    }
    ...
    if (e.target.id === "shuffle-button") {
      ...
      if (target) {
        ...
        // Re-run the swatches on the newly shuffled colors!
        updateThemeSwatches();
      }
    }
    ...
    if (e.target.id === "apply-preset-button") {
      ...
      // If the input was successfully loaded, refresh our display
      if (ok) {
        var target = document.getElementById("get-css-theme");
        if (target) {
          target.textContent = getThemeCSS();

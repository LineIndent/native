...
//   var state = { base: null, color: null, radius: null };
...
//     if (state.color === null) state.color = registries.color[0].id;
//     if (state.radius === null) state.radius = registries.radius[0].id;
//     return true;
//   }

//   function applyPalette(name) {
//     var entry = findById(name, state[name]);
//     if (!entry) return;
//     var root = document.querySelector(".preview-theme");
//     if (!root) return;
...
//     var root = document.querySelector(".preview-theme");
//     if (!root) return;
//     root.style.setProperty("--radius", entry.value);
//   }
...
//     var color = findById("color", state.color);
//     var radius = findById("radius", state.radius);
//     if (!base || !color || !radius) return "";

//     var light = Object.assign({}, base.light, color.light, { radius: radius.value });
//     var dark = Object.assign({}, base.dark, color.dark, { radius: radius.value });
...
//   window.preview = {
//     state: state,
//     setBase: function (id) { state.base = id; applyAll(); },
//     setColor: function (id) { state.color = id; applyAll(); },
//     setRadius: function (id) { state.radius = id; applyRadius(); },
//     applyAll: applyAll,
//     getThemeCSS: getThemeCSS,
//   };

//   // Event delegation on document — works no matter how many times the
//   // theme builder page mounts/unmounts, since we never touch the specific
//   // select/button elements directly or need to "re-wire" anything on
//   // remount. This file itself only ever runs once (real <script src>,
//   // not something re-executed by a component lifecycle).
//   document.addEventListener("change", function (e) {
//     if (e.target.id === "base-theme-select") window.preview.setBase(e.target.value);
//     if (e.target.id === "color-theme-select") window.preview.setColor(e.target.value);
//     if (e.target.id === "radius-select") window.preview.setRadius(e.target.value);
//   });
...
//   // Watch the dark class directly instead of requiring every toggle
//   // mechanism in the app to remember to call window.preview.applyAll().
//   // Any code, anywhere, that flips `dark` on <html> now refreshes the

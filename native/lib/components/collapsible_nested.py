...
def file_item(name: str, root_level: bool = False) -> rx.Component:
    ...
    return rx.el.div(
        ...
        class_name=f"flex items-center gap-2 {padding_class} py-1 hover:bg-muted/40 rounded-md cursor-pointer",
    )
...
def folder_item(name: str, *children: rx.Component) -> rx.Component:
    return collapsible.root(
        collapsible.trigger(
            ...
            hi(
                "ArrowRight01Icon",
                class_name="size-3.5 ml-auto text-muted-foreground/70 [details[open]>summary_&]:rotate-90",
            ),
            class_name="w-full justify-start gap-2 h-8 px-2 rounded-md font-normal hover:bg-muted/60",
        ),
        collapsible.panel(
            rx.el.div(
                *children,
